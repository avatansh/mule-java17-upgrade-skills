...
// ── stage=test ────────────────────────────────────────────────────────────────────────────
test("test failure → MUNIT_FAILED (lock NOT released)", () => {
  const store = makeStore({ j1: { jobId: "j1", appName: "a", status: "PR_OPEN" } });
  const events = [];
  const r = ingestCiResult(
...
test("test unknown result → 400", () => {
  const store = makeStore({ j1: { jobId: "j1", status: "PR_OPEN" } });
  const r = ingestCiResult({ jobId: "j1", stage: "test", result: "weird" }, { store });
  assert.equal(r.statusCode, 400);
});

// ── stage=dependency-guard ──────────────────────────────────────────────────────────────────
test("dep-guard failure → DEP_GUARD_FAILED with report", () => {
  const store = makeStore({ j1: { jobId: "j1", appName: "a", status: "PR_OPEN" } });
  const report = [{ groupId: "com.x", artifactId: "y", resolvedVersion: "1.0.0", requiredVersion: "2.0.0" }];
  const r = ingestCiResult(
    { jobId: "j1", stage: "dependency-guard", result: "failure", report },
    { store }
  );
...
test("dep-guard success resumes DEP_GUARD_FAILED → PR_OPEN", () => {
  const store = makeStore({ j1: { jobId: "j1", appName: "a", status: "DEP_GUARD_FAILED" } });
  const r = ingestCiResult({ jobId: "j1", stage: "dependency-guard", result: "success" }, { store });
  assert.equal(r.response.resumed, true);
  assert.equal(store.getJob("j1").status, "PR_OPEN");
  assert.equal(store.getJob("j1").depGuard.result, "passed");
});

test("dep-guard success note-only when not parked", () => {
  const store = makeStore({ j1: { jobId: "j1", appName: "a", status: "PR_OPEN" } });
  const r = ingestCiResult({ jobId: "j1", stage: "dependency-guard", result: "success" }, { store });
...
// ── stage=deploy ──────────────────────────────────────────────────────────────────────────
test("deploy success + no verifier → DEPLOYED (trust CI); lock released", () => {
  const store = makeStore({ j1: { jobId: "j1", appName: "a", status: "DEPLOYING" } });
  const r = ingestCiResult({ jobId: "j1", stage: "deploy", result: "success", deployUrl: "u" }, { store });
...
test("deploy success + verifier healthy → DEPLOYED verified", () => {
  const store = makeStore({ j1: { jobId: "j1", appName: "a", status: "DEPLOYING" } });
  const verifyDeploy = () => ({ status: "healthy", platform: { status: "RUNNING" } });
  const r = ingestCiResult({ jobId: "j1", stage: "deploy", result: "success" }, { store, verifyDeploy });
  assert.equal(store.getJob("j1").status, "DEPLOYED");
  assert.equal(store.getJob("j1").platformVerified, true);
  assert.equal(store.getJob("j1").platformStatus, "RUNNING");
});

test("deploy success + verifier unhealthy → FAILED_DEPLOY (discrepancy)", () => {
  const store = makeStore({ j1: { jobId: "j1", appName: "a", status: "DEPLOYING" } });
...
test("deploy failure → FAILED_DEPLOY + rolledBack; lock released", () => {
  const store = makeStore({ j1: { jobId: "j1", appName: "a", status: "DEPLOYING" } });
  const r = ingestCiResult({ jobId: "j1", stage: "deploy", result: "failure", error: "boom" }, { store });
  assert.equal(store.getJob("j1").status, "FAILED_DEPLOY");
...
test("deploy idempotent when already DEPLOYED", () => {
  const store = makeStore({ j1: { jobId: "j1", appName: "a", status: "DEPLOYED" } });
  const r = ingestCiResult({ jobId: "j1", stage: "deploy", result: "success" }, { store });
  assert.equal(r.response.idempotent, true);
  assert.deepEqual(store.released, []); // no double release
});

test("default stage is deploy", () => {
  const store = makeStore({ j1: { jobId: "j1", appName: "a", status: "DEPLOYING" } });
  const r = ingestCiResult({ jobId: "j1", result: "success" }, { store });
  assert.equal(store.getJob("j1").status, "DEPLOYED");
});

// ── guards ──────────────────────────────────────────────────────────────────────────────────
test("missing jobId → 400", () => {
  const store = makeStore({});
  const r = ingestCiResult({ result: "success" }, { store });
  assert.equal(r.statusCode, 400);
});

test("unknown jobId → 404", () => {
  const store = makeStore({});
  const r = ingestCiResult({ jobId: "nope", result: "success" }, { store });
